...
from src.app.domain.accounts.services import RoleService, UserRoleService, UserService
...
async def provide_roles_service(
    ...
    async with RoleService.new(
        session=db_session,
        load=selectinload(Role.users).options(
            joinedload(UserRole.user, innerjoin=True)
        ),
    ) as service:
        yield service


async def provide_user_roles_service(
    db_session: AsyncSession | None = None,
